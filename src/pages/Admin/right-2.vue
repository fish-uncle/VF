	...
	<div>
		...
		<div class="vf-control" v-if="item.changeList.indexOf('group')!==-1">
			...
			<i-select :clearable="true"
					  v-model="item.group">
				<i-option :value="item.id" v-for="item in center.group" :key="item.id">{{item.key}}</i-option>
			</i-select>
		</div>
		...
		<div class="vf-control" v-if="item.changeList.indexOf('selectList')!==-1">
			...
			<ul class="vf-select-list" v-if="tab===1">
				...
				<li class="fn-flex flex-row" v-for="(child,index) in item.selectList" :key="child.value">
		...
		<div class="vf-control" v-if="item.changeList.indexOf('controlOthersHide')!==-1">
			...
			<ul v-if="item.selectListUrl">
				<li v-for="child in item.ajaxList" :key="child.value">
			...
			<ul v-if="!item.selectListUrl">
				<li v-for="child in item.selectList" :key="child.value">
					<span>{{$t('admin_right_btn22_1')}} <strong>{{child.label}}</strong> {{$t('admin_right_btn22_2')}}</span>
					<Transfer
						not-found-text=""
						:data="componentList"
